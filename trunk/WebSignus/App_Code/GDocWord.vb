...
#End Region
                ...
                If dtConfiguracion.Rows(k)("TIP_DAT").ToString <> "I" Then
                    ...
                    Do While no_encontrado < 3
                        ...
                        If oWrdApp.Selection.Find.Found() Then
                            ...
                            wrdMergeFields.Add(wrdRng, vHeader(k))
                        Else
    ...
    Private Function CrearBDCorrespondencia(ByRef oWrdDoc As MSWord._Document, ByRef oWrdApp As MSWord.Application, ByVal dtConfiguracion As DataTable, ByVal dtDatosImprimir As DataTable) As String
        ...
        'oWrdBD = oWrdApp.Documents.Add()
        'Dim NFilas As Integer = dtDatosImprimir.Rows.Count
        'Dim NCols As Integer = dtConfiguracion.Rows.Count
        'oTable = oWrdBD.Tables.Add(oWrdBD.Range, NFilas, NCols, oMissing, oMissing)

        Dim CamposAl As String = ""
        ''SE QUITA LA CREACION NORMAL
        ...
        '            CamposAl = CamposAl + "," + Chr(65 + k)
        '        End If

        '        'oTable.Cell(1, k).Range.Text = dtConfiguracion.Rows(k)("NOM_PLA").ToString
        '    End If
        'Next
        'Se crean las columnas con la seria A,B,C,..AA,AB,AC..
        CamposAl = Util.CrearHeader(dtConfiguracion.Rows.Count)
        vHeader = CamposAl.Split(",")

        'oWrdDoc.MailMerge.CreateDataSource(Name:=PathTemporal, HeaderRecord:=Campos)
        oWrdDoc.MailMerge.CreateDataSource(Name:=PathTemporal, HeaderRecord:=CamposAl)
        'oWrdDoc.MailMerge.OpenDataSource(Name:=PathTemporal)
